 ...
 * ```
 */
export class OrderedEventEmitter<L extends ListenerSignature<L> = DefaultListener> {
  listenersMap: Map<keyof L, { listener: L[keyof L]; priority: number; key?: string }[]>;
  constructor() {
   ...
   */
  addListener<U extends keyof L>(event: U, listener: L[U], priority: number, key?: string): this {
   ...
   */
  on<U extends keyof L>(event: U, listener: L[U], priority: number, key?: string): this {
  ...
  emit<U extends keyof L>(
    ...
    const zeroPriorityListeners = listeners.filter((listener) => listener.priority === 0);
    ...
    const nonZeroPriorityListeners = listeners.filter((listener) => listener.priority !== 0);
